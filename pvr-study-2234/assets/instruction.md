<div style="max-width: 1000px; margin: 0 auto; padding: 0 24px; box-sizing: border-box;">

## Instructions

<ol style="margin: 0; padding-left: 0; list-style-position: inside;">
<li style="margin-bottom: 6px;">You will see bar charts and estimate roughly the ratio of the taller bar’s height to the shorter bar’s height (i.e., how many times taller the taller bar is than the shorter one). In the example below, the ratio is about 2.
<figure style="margin: 8px 0 2px 0;">
<img src="./pvr-study-1/assets/instruct-chart-example.png" alt="Example bar chart with a 2 to 1 ratio." style="width: 500px; height: auto;" />
</figure>
</li>
<li style="margin-bottom: 6px;">If a bar is too tall for the screen, scroll up to see it. You can scroll only upward but back down is not possible. Sometimes you may need to scroll for a long time; that is normal.</li>
<li style="margin-bottom: 6px;"><span style="color:red"><strong>Do not count the number of scrolls</strong></span> you make to estimate the ratio. Also, <span style="color:red"><strong>do not use any other counting or calculation method, or external tools. Just give quick, intuitive responses.</strong></span></li>
<li style="margin-bottom: 6px;">We are not looking for precise answers; payment is not based on accuracy. However, clearly random input (e.g., the same answer every time, or values like 1 when bars differ greatly) is not eligible for payment.</li>
<li style="margin-bottom: 6px;">Do not skip large parts of the chart in one step (e.g., Page Up or End).</li>
<li style="margin-bottom: 6px;">If you make a typo in your answer, you can revise and re-submit it before moving on.</li>
</ol>

</div>
